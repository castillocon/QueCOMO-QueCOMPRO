    ...
    const RecipeFormPage: React.FC = () => {
      ...
      const form = useForm<RecipeFormValues>({
        ...
        defaultValues: {
          ...
          description: '',
          mealtype: 'Almuerzo',
          ingredients: [{ name: '', quantity: '', supplier: '' }] as Ingredient[],
          instructions: [''],
          imageUrl: '',
        },
      });
      ...
      const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
          ...
          if (file.size > MAX_FILE_SIZE) {
            toast.error(`El tamaño máximo de la imagen es 5MB. Tu imagen es de ${(file.size / (1024 * 1024)).toFixed(2)}MB.`);
            event.target.value = ''; // Clear the input
            setSelectedFile(null);
            setImagePreview(null);
            form.setValue('image', undefined);
      ...
      const onSubmit = async (values: RecipeFormValues) => {
        ...
        if (isEditing && currentRecipe) {
          await updateRecipe(
            { ...currentRecipe, ...recipeData, imageUrl: values.imageUrl } as Recipe,
            selectedFile === null && currentRecipe.imageUrl ? null : selectedFile
          );
        } else {
